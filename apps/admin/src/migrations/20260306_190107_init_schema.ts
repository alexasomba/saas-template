...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`CREATE INDEX \`variants_rels_parent_idx\` ON \`variants_rels\` (\`parent_id\`);`,
  );
  ...
  await db.run(
    sql`CREATE INDEX \`products_gallery_variant_option_idx\` ON \`products_gallery\` (\`variant_option_id\`);`,
  );
  ...
  await db.run(
    sql`CREATE INDEX \`_products_v_version_gallery_variant_option_idx\` ON \`_products_v_version_gallery\` (\`variant_option_id\`);`,
  );
  ...
  await db.run(
    sql`CREATE INDEX \`carts_items_parent_id_idx\` ON \`carts_items\` (\`_parent_id\`);`,
  );
  ...
  await db.run(
    sql`CREATE INDEX \`orders_items_product_idx\` ON \`orders_items\` (\`product_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`orders_items_variant_idx\` ON \`orders_items\` (\`variant_id\`);`,
  );
